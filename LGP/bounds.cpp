...
PathBound::PathBound(ptr<KOMO>& komo,
  ...
  if(collisions) komo->add_collision(true, 0., 1e2);
  komo->add_jointLimits(true);
...
SeqPathBound::SeqPathBound(ptr<KOMO>& komo,
  ...
  double maxPhase = getMaxPhase(S);
  komo->clearObjectives();
  std::cout << "SeqPathBound " << "\n";
  komo->setModel(startKinematics, collisions);
  uint stepsPerPhase = rai::getParameter<uint>("LGP/stepsPerPhase", 20);
  uint pathOrder = rai::getParameter<uint>("LGP/pathOrder", 2);
  komo->setTiming(maxPhase+.5, stepsPerPhase, 5., pathOrder);
  komo->animateOptimization = 0;
  ...
  if(collisions) komo->add_collision(true, 0., 1e4);
  komo->add_jointLimits(true);

  komo->run_prepare(.01);
  std::cout << "waypoints " << waypoints << "\n";
  std::cout << "waypointsStepsPerPhase " << waypointsStepsPerPhase << "\n";
