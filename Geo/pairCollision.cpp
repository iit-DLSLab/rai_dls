    ...
    : mesh1(&_mesh1), mesh2(&_mesh2), t1(&_t1), t2(&_t2), rad1(rad1), rad2(rad2)
{
  ...
  rai::Mesh M1(*mesh1);
  if (!t1->isZero())
    t1->applyOnPointArray(M1.V);
  rai::Mesh M2(*mesh2);
  if (!t2->isZero())
    t2->applyOnPointArray(M2.V);

  libccd(M1, M2, _ccdGJKIntersect);
std::cout << "Calling GJK_sqrDistance()" << std::endl;
#else
  ...
  GJK_sqrDistance();
            ...
            // auto end_time = td::chrono::duration_cast<std::chrono::milliseconds>(end);
            // std::cout << "end: " << end_time.count() << std::endl;

            // Compute the difference between the end and start times
            // Convert time to milliseconds
            auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);

            std::cout << "time GJK: " << duration.count() << " milliseconds" << std::endl;
#endif
...
#ifndef FCLmode
  if (distance < 1e-10)
  { // WARNING: Setting this to zero does not work when using
    // THIS IS COSTLY! DO WITHIN THE SUPPORT FUNCTION?
    rai::Mesh M1(*mesh1);
    if (!t1->isZero())
      t1->applyOnPointArray(M1.V);
  ...
  { // exact touch: the GJK computed things, let's make them consisten
    p1 = p2 = .5 * (p1 + p2);
  }
...
PairCollision::PairCollision(ScalarFunction func1, ScalarFunction func2, const arr &seed)
{
  ...
  arr x = seed;
  CHECK_EQ(x.N, 3, "");
  OptNewton newton(x, f, OptOptions().set_verbose(0).set_stopTolerance(1e-4).set_maxStep(1.).set_damping(1e-10));
  newton.run();
  ...
  if (d1 < d2)
  { // deeper into d1 -- use g1 as normal!
    normal = g1;
  ...
  simplex1 = p1;
  simplex1.reshape(1, 3);
  simplex2 = p2;
  simplex2.reshape(1, 3);
}
...
void _getSimplex(arr &S, ccd_vec3_t *simplex, const arr &mean)
{
  ...
  for (uint i = 0; i < 4; i++)
  {
    ...
    if (!_equal(s, s))
      continue; // don't append nan!
    if (_equal(s, mean.p))
      continue;
    sel = true;
    for (uint j = 0; j < i; j++)
      if (_approxEqual(s, simplex[j].v))
      {
        sel = false;
        break;
      }
...
void PairCollision::libccd(rai::Mesh &m1, rai::Mesh &m2, CCDmethod method)
{
  ...
  if (method == _ccdMPRPenetration)
  {
    ...
    if (distance > -1e-10)
      return; // minimal penetration -> simplices below are not robust

    // grab simplex points
    if (m1.V.d0 == 1)
      simplex1 = m1.V; // m1 is a point/sphere
    else
      _getSimplex(simplex1, simplex, m1.getMean());
    if (m2.V.d0 == 1)
      simplex2 = m2.V; // m2 is a point/sphere
    else
      _getSimplex(simplex2, simplex + 4, m2.getMean());
    if (simplex1.d0 > 3)
      simplex1.resizeCopy(3, 3);
    if (simplex2.d0 > 3)
      simplex2.resizeCopy(3, 3);
  }
  ...
  else if (simplexType(3, 3))
  {
    ...
    HALT("simplex types " << simplex1.d0 << ' ' << simplex2.d0 << " not handled");
  CHECK_EQ(p1.N, 3, "PairCollision failed")
  CHECK_EQ(p2.N, 3, "PairCollision failed")
...
std::cout << "GJK comp " << "\n";
  ...
  m1.numpoints = mesh1->V.d0;
  m1.vertices = mesh1->V.getCarray(Vhelp1);
  m1.rings = nullptr; // TODO: rings would make it faster
  m2.numpoints = mesh2->V.d0;
  m2.vertices = mesh2->V.getCarray(Vhelp2);
  m2.rings = nullptr;
  ...
  if (!!t1)
  {
    T1 = t1->getAffineMatrix();
    T1.getCarray(Thelp1);
  }
  if (!!t2)
  {
    T2 = t2->getAffineMatrix();
    T2.getCarray(Thelp2);
  }
  ...
  if (simplex.npts >= 2)
  {
    if (simplex.simplex1[1] != simplex.simplex1[0])
      simplex1.append(arr(simplex.coords1[1], 3, true));
    if (simplex.simplex2[1] != simplex.simplex2[0])
      simplex2.append(arr(simplex.coords2[1], 3, true));
  }
  if (simplex.npts >= 3)
  {
    if (simplex.simplex1[2] != simplex.simplex1[0] && simplex.simplex1[2] != simplex.simplex1[1])
      simplex1.append(arr(simplex.coords1[2], 3, true));
    if (simplex.simplex2[2] != simplex.simplex2[0] && simplex.simplex2[2] != simplex.simplex2[1])
      simplex2.append(arr(simplex.coords2[2], 3, true));
  }
#else
  NICO
#endif
                                ...
                                const arr &Jp1, const arr &Jp2)
{
  ...
  std::cout << "distance- rad1 - rad2" << distance- 2.0*(rad1 - rad2) << "\n";

  y = ARR(distance - (rad1 + rad2)*2.0);
  if (!!J)
  {
    arr Jdiff = Jp1 - Jp2;
    J = ~normal * Jdiff;
                              ...
                              const arr &Jx1, const arr &Jx2)
{
  ...
  if (!!J)
  {
    ...
    {
      arr a = simplex1[1] - simplex1[0];
      a /= length(a);
      arr b = simplex2[1] - simplex2[0];
      b /= length(b);
      double ab = scalarProduct(a, b);
      if (1. - ab * ab > 1e-8)
      { // the edges are not colinear
...
void PairCollision::kinPointP1(arr &y, arr &J, const arr &Jp1, const arr &Jp2, const arr &Jx1, const arr &Jx2)
{
  ...
  if (!!J)
  {
    ...
    if (simplexType(2, 2))
    {
      ...
      if (fabs(ac) > 1e-10)
      { // otherwise the Jacobian is singular...
        J += ((1. / ac) * (a ^ c)) * (Jp2 - Jp1);
...
void PairCollision::kinPointP2(arr &y, arr &J, const arr &Jp1, const arr &Jp2, const arr &Jx1, const arr &Jx2)
{
  ...
  if (!!J)
  {
    ...
    if (simplexType(2, 2))
    {
      arr a = simplex2[1] - simplex2[0];
      a /= length(a);
      arr b = simplex1[1] - simplex1[0];
      b /= length(b);
      double ab = scalarProduct(a, b);
...
void PairCollision::nearSupportAnalysis(double eps)
{
  ...
  arr S1, S2;
  for (uint i : pts1)
    S1.append(M1.V[i] - rad1 * normal);
  for (uint i : pts2)
    S2.append(M2.V[i] + rad2 * normal);
  S1.reshape(pts1.N, 3);
  S2.reshape(pts2.N, 3);
