      ...
      : mode(_mode), excludes(_excludes)
  {
    ...
    for (long int i = 0; i < geometries.N; i++)
    {
      ...
      if (shape)
      {
        ...
        {
          geom = make_shared<fcl::Capsulef>(shape->size(-1), shape->size(-2));
        }
        else if (shape->type() == ST_cylinder)
        {
          geom = make_shared<fcl::Cylinderf>(shape->size(-1), shape->size(-2));
        }
        else if (shape->type() == ST_sphere)
        {
          geom = make_shared<fcl::Spheref>(shape->size(-1));
        ...
        model->endModel();
          ...
          for (uint i = 0; i < dat->polygons.d0; i++)
            dat->polygons(i, 0) = 3;
// #if FCL_MINOR_VERSION >= 7
          auto verts = std::make_shared<std::vector<fcl::Vector3<float>>>(mesh.V.d0);
          auto faces = std::make_shared<std::vector<int>>();

          // Populate the verts vector with mesh vertices
          for (uint i = 0; i < verts->size(); ++i)
          {
            (*verts)[i] = fcl::Vector3<float>(
                static_cast<float>(mesh.V(i, 0)),
                static_cast<float>(mesh.V(i, 1)),
                static_cast<float>(mesh.V(i, 2)));
          }

          // Populate the faces vector with mesh face data
          // The faces vector should encode the faces in the required format
          // For each face: [number of vertices, index1, index2, index3, ...]
          for (uint i = 0; i < mesh.T.d0; ++i)
          {
            // Assuming each face is a triangle
            faces->push_back(3); // Number of vertices in the face
            faces->push_back(mesh.T(i, 0));
            faces->push_back(mesh.T(i, 1));
            faces->push_back(mesh.T(i, 2));
          }
// #else
//           geom = std::make_shared<fcl::Convex<float>>(verts, mesh.T.d0, faces, true);
...
// #endif
  ...
  bool FclInterface_self::BroadphaseCallback(CollObject *o1, CollObject *o2, void *cdata_)
  {
    ...
    if (fcl->excludes.N)
    {
      if (a < b)
      {
        if (fcl->excludes(a).containsInSorted(b))
          return false;
      }
      else
      {
        if (fcl->excludes(b).containsInSorted(a))
          return false;
      }
    }
    ...
    else if (fcl->mode == fcl->_binaryCollisionSingle || fcl->mode == fcl->_binaryCollisionAll)
    { // fine boolean collision query
      ...
      {
        fcl->addCollision(a, b);
        if (fcl->mode == fcl->_binaryCollisionSingle)
    ...
    else if (fcl->mode == fcl->_distanceCutoff)
    {
      ...
      if (result.min_distance < fcl->cutoff)
        fcl->addCollision(a, b);
    }
    else
    {
...
#else // RAI_FCL
rai::FclInterface::FclInterface(const rai::Array<shared_ptr<Mesh>> &_geometries, double _cutoff){NICO} rai::FclInterface::~FclInterface() { NICO }
