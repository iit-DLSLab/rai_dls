...
struct F_PairCollision : Feature {
  ...
  F_PairCollision() : type(_negScalar), neglectRadii(false) {};
...
struct F_PairCollisionInequality : Feature {
  enum Type { _none=-1, _negScalar, _vector, _normal, _center, _p1, _p2 };
  ...
  F_PairCollisionInequality() : type(_negScalar), neglectRadii(false) {};


  F_PairCollisionInequality(Type _type, bool _neglectRadii=false)
    : type(_type), neglectRadii(_neglectRadii) {
  }
  virtual void phi2(arr& y, arr& J, const FrameL& F);
  virtual uint dim_phi2(const FrameL& F){  if(type==_negScalar) return 1; return 3;  }
