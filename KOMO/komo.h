...
// #include "networks_minimal/FeasibilityMargin.hpp"
// #include "networks_minimal/Activation.hpp"
// #include "networks_minimal/Transform.hpp"
// #include "networks_minimal/FeasibilityMargin.hpp"
// #include "networks_minimal/Activation.hpp"
// #include "networks_minimal/Transform.hpp"
//===========================================================================
...
struct KOMO : NonCopyable
{
  ...
  arr R_OG;
  void setR_OG(const arr &newR_OG)
  {
    R_OG = newR_OG; // you can add more complex checks or operations here
  }
  ...
  //
  ptr<Objective> add_qControlObjectiveAcceleration(const arr &times, arr scaleFactors);
  ptr<Objective> add_qControlObjectivePosition(const arr &times, arr scaleFactors);
  ...
  void add_pair_collision(double margin = .0, double prec = 1e1);
  void add_jointLimits(bool hardConstraint = true, double margin = .0, double prec = 1.);
  // void add_parabola(bool hardConstraint = true, double margin = .0, double prec = 1.);
  // void add_feasibilityMargin(bool hardConstraint = true, double margin  = .0, double prec = 1.);
  // void add_feasibilityMarginConstraint(bool hardConstraint = true, double margin  = .0, double prec = 1.);
...
public:
  ...
  {
    DEPR;
    return view_play(watch, delay, saveVideoPath);
  }
  bool displayPath(const char *txt, bool watch = true, bool full = true)
  {
    DEPR;
    return view(watch, txt);
  }
  ...
  {
    DEPR;
    HALT("see komo-21-03-06");
  }
  void deactivateCollisions(const char *s1, const char *s2);
  arr getFrameStateX(int t)
  {
    DEPR;
    return getConfiguration_X(t);
  }
  arr getPath_qAll(int t)
  {
    DEPR;
    return getConfiguration_qOrg(t);
  }
  arr getConfiguration_q(int t)
  {
    DEPR;
    return getConfiguration_qAll(t);
  }
  arr getPath_qOrg(uintA joints, const bool activesOnly)
  {
    DEPR;
    return getPath_qOrg();
  }
